...
function Journal() {
  const articles = [["changing-initial-jewelry","01","CARE","When is the right time to change the initial jewelry?","It is the condition of the tissue, not only the number of weeks, that matters."],["titanium-vs-surgical-steel","02","MATERIAL","ASTM F-136 titanium vs. surgical steel","What implant-grade means and why we use it for fresh piercings."],["first-piercing-step-by-step","03","GUIDE","Your first piercing, step by step","Consultation, placement, application and the first days of healing."],["normal-during-healing","04","HEALING","What is normal during healing?","Swelling, crusting and tenderness — when to wait and when to contact us."],["piercing-for-minors","05","MINORS","Piercing for minors","What to bring, who must attend and why rules differ by piercing type."],["anatomy-and-piercing","06","ANATOMY","Why not every piercing suits everyone","How anatomy affects placement, jewelry choice and long-term results."]];
  return <PageShell lang="en" slug="journal" eyebrow="05 / JOURNAL" title={<>FIELD<br />NOTES.</>} intro="Practical, myth-free information about materials, healing, anatomy and safe decisions."><section className="article-index frame">{articles.map(([slug,n,cat,title,text])=><Link href={`/en/journal/${slug}`} key={slug}><span>{n} / {cat}</span><h2>{title}</h2><p>{text}</p><b>READ / 05 MIN <ArrowIcon /></b></Link>)}</section></PageShell>;
}
