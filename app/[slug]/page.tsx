...
function Studio() {
  ...
  return <PageShell lang="cs" slug="studio" eyebrow="01 / STUDIO" title={<>KONTROLOVANÉ<br />DO DETAILU.</>} intro="Prostor, lidé a procesy navržené kolem jednoho cíle: bezpečný piercing, který s vámi dlouhodobě funguje.">
    ...
    <section id="team" className="sub-section frame"><SectionIndex index="01" label="TÝM" /><h2>Dva lidé.<br />Jeden standard.</h2><div className="bio-grid"><article><div className="portrait monogram">A</div><span>ZAKLADATEL / PIERCER</span><h3>Alex</h3><p>Zakladatel Black Lotus. Profesionálně se piercingu věnuje od roku 2015; bezpečnost, přesné umístění a klid klienta jsou základem jeho práce.</p></article><article><div className="portrait monogram">P</div><span>PIERCER</span><h3>Paxy</h3><p>Individuální konzultace, cit pro styling a důsledná péče před aplikací i během hojení.</p></article></div></section>
...
function Journal() {
  const articles = [["vymena-prvniho-sperku","01","PÉČE","Kdy je správný čas vyměnit první šperk?","Ne počet týdnů, ale stav tkáně. Jak poznat, že je čas na kontrolu."],["titan-vs-chirurgicka-ocel","02","MATERIÁL","Titan ASTM F-136 vs. chirurgická ocel","Co znamená implant-grade standard a proč ho používáme pro čerstvý piercing."],["prvni-piercing-krok-za-krokem","03","PRŮVODCE","První piercing krok za krokem","Konzultace, umístění, aplikace a první dny hojení bez nejasností."],["co-je-normalni-pri-hojeni","04","HOJENÍ","Co je normální během hojení?","Otok, krusty a citlivost — kdy vyčkat a kdy se ozvat."],["piercing-u-nezletilych","05","NEZLETILÍ","Piercing u nezletilých","Co přinést, kdo musí přijít a proč se pravidla liší podle typu piercingu."],["proc-neni-kazdy-piercing-pro-kazdeho","06","ANATOMIE","Proč není každý piercing pro každého","Jak anatomie ovlivňuje umístění, výběr šperku i dlouhodobý výsledek."]];
  return <PageShell lang="cs" slug="journal" eyebrow="05 / ČLÁNKY" title={<>POZNÁMKY<br />Z PRAXE.</>} intro="Praktické informace bez mýtů — o materiálech, hojení, anatomii a bezpečném rozhodování."><section className="article-index frame">{articles.map(([slug,n,cat,title,text])=><Link href={`/journal/${slug}`} key={slug}><span>{n} / {cat}</span><h2>{title}</h2><p>{text}</p><b>ČÍST / 05 MIN <ArrowIcon /></b></Link>)}</section></PageShell>;
}
